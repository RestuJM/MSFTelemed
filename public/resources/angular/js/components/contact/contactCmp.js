angular.module('sample').component('rbxContactCall', {
    bindings: {
        name: '@'
    },
    controller : function rbcConnectionCtrl(rainbowSDK, $rootScope, $scope) {
        ...
        $scope.contacts = [{_displayName: "No recipient", id: "no", status:"offline", avatar: {src:null}}];
        ...
        this.$onInit = function() {
            ...
            listeners.push($rootScope.$on(rainbowSDK.connection.RAINBOW_ONSTARTED, onStart));
            ...
            // Subscribe to contact change
            listeners.push($rootScope.$on(rainbowSDK.contacts.RAINBOW_ONINFORMATIONCHANGED, onContactsInformationChanged));
        };
        ...
        var onContactsInformationChanged = function onContactsInformationChanged(__event, contact) {
        ...
        var changeRecipientValue = function changeRecipientValue() {
            if ($scope.selectedItem) {
                ...
                $scope.actionAudio = $scope.presenceValue === "notAvailable" ? "Not Available" : $scope.presenceValue === "busy" ? "Busy" : "Audio call";
                $scope.actionVideo = $scope.presenceValue === "notAvailable" ? "Not Available" : $scope.presenceValue === "busy" ? "Busy" : "Call";
                $scope.isAvailable = $scope.presenceValue === "available" ? true : false;    
            }
        };
